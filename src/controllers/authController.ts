...
import {ErrorHandler} from "../err/errroHandller";

class AuthController {
    public async registration(req:Request, res:Response, next: NextFunction) {
        try {
            const createdUser = await userService.createUser(req.body);
            const { accessToken, refreshToken } = await authService.generateTokenPair(createdUser);
            res.json({
                token: {
                    accessToken,
                    refreshToken,
    ...
    public async login(req:IRequestExtended, res:Response, next:NextFunction) {
        try {
            ...
            const passwordMatched = await userService.compareUserPassword(password, hashPassword);
            if (!passwordMatched){
